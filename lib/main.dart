...
void main() { WidgetsFlutterBinding.ensureInitialized(); runApp(const HomeTourApp()); }
...
class HomeTourApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ChangeNotifierProvider(
    ...
    child: MaterialApp(
      ...
      theme: ThemeData(useMaterial3: true, colorSchemeSeed: const Color(0xFF15803D)),
      home: const AppShell(),
    ),
  );
}
...
class _AppShellState extends State<AppShell> {
  int index = 0;
  final pages = const [HomeScreen(), ToursScreen(), BookingsScreen(), ProfileScreen()];
  @override void initState() { super.initState(); WidgetsBinding.instance.addPostFrameCallback((_) => context.read<AppState>().bootstrap()); }
  @override Widget build(BuildContext context) {
    final state = context.watch<AppState>();
    if (state.loading) return const Scaffold(body: Center(child: CircularProgressIndicator()));
    if (!state.authenticated) return LoginScreen(onLoggedIn: () => setState(() {}));
    return Scaffold(
